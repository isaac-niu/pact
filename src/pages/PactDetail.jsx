...
export default function PactDetail() {
  ...
  async function takeFiles(list) {
    ...
    setBusy(true);
    try {
      if (liveSlip) await live.submitLiveEvidence(pact, raw.length === 1 ? raw[0] : raw);
      else await submitEvidence(pact.id, raw.length === 1 ? raw[0] : raw);
    } catch (err) {
  ...
  async function postSignal(raw) {
    ...
    setBusy(true);
    try {
      if (liveSlip) throw new Error("Live 1v1s take a photo on this desk.");
    ...
    <div>
      ...
      <div className="detail-grid">
        <div className="card">
          ...
          {canUpload && !liveSlip ? <ProofSignals busy={busy} onLocation={onLocation} onFitness={onFitness} /> : null}
